...
/**
 * Nanobenchmark: META
 *   MU. PROCESS = {overwrite an overlapping region of /test/test.file}
 *       - TEST: fallback to spinlock
 */	      
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
{
        struct fx_opt *fx_opt = fx_opt_worker(worker);
        sprintf(test_root, "%s/n_fallback.dat", fx_opt->root);
}
...
struct bench_operations n_fallback_ops = {
        .pre_work  = pre_work, 
        .main_work = main_work,
};
