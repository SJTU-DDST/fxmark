...
static struct bench_desc bench_table[] = {
	...
	{"DWOH",
	 ...
	 &n_fallback_ops},
	{"filetest",
	 "from SoupFS, mixed create and unlink",
	 &n_filetest_ops},
	{"MMAPL",
	 "each process mmap and munmap a private file",
	 &n_MMAPL_ops},
	{"MMAPM",
	 "each process mmap and munmap its private region of the shared file",
	 &n_MMAPM_ops},
	{"MMAPH",
	 "each process mmap and munmap the same page of the shared file",
	 &n_MMAPH_ops},
	{"DWOM_rand",
	 "mtime update: each process updates random private pages of the shared file",
	 &n_DWOM_rand_ops},
	{"pagefault",
	 "pagefault",
	 &n_pagefault_ops},
